  ...
  return (
    <div className="bg-raised rounded-xl border border-hair mb-6">
      <div className="px-6 py-4 border-b border-hair">
        <h3 className="text-base font-semibold text-ink">Module insights</h3>
          ...
          return (
            <div key={mn} className="px-6 py-4">
              <div className="flex items-baseline gap-2 mb-1 flex-wrap">
                <span className="text-xs font-mono text-faint">M{mn}</span>
                <h4 className="text-sm font-semibold text-ink">{moduleName}</h4>
              ...
              {meta && (
                <p className="text-[11px] text-muted mb-3 italic">
                  ...
                  <span className="text-faint"> · &ldquo;{meta.oneLiner}&rdquo;</span>
                </p>
              )}
              ...
              <div className="space-y-2">
                {entries.map((e) => {
                  ...
                  const colorClass = LEVEL_COLOR[score] ?? "bg-surface text-ink";
                  return (
                    <details
                      key={`${e.stakeholder_id}-${mn}`}
                      className="rounded-lg border border-hair bg-paper hover:border-hair transition-colors"
                    >
                      <summary className="cursor-pointer px-4 py-2.5 flex items-center justify-between flex-wrap gap-2">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <span className="text-sm text-ink truncate">
                            {e.stakeholder_name}
                            <span className="text-xs text-muted ml-1.5">— {e.stakeholder_role}</span>
                          </span>
                        </div>
                        <span className="text-[11px] text-faint">Click to read narrative →</span>
                      </summary>

                      <div className="px-4 pb-4 pt-1">
                        {e.narrative && (
                          <div className="mb-3">
                            <p className="text-[10px] uppercase tracking-wider text-muted mb-1">Why this score</p>
                            <p className="text-sm text-ink leading-relaxed">{e.narrative}</p>
                          </div>
                        )}

                        {!e.narrative && e.evidence && (
                          <div className="mb-3">
                            <p className="text-[10px] uppercase tracking-wider text-muted mb-1">Evidence</p>
                            <p className="text-xs text-ink">{e.evidence}</p>
                          </div>
                        )}
                            ...
                            <ol className="space-y-2">
                              {e.path_to_next_level.map((step, i) => (
                                <li key={i} className="flex gap-3 text-sm">
                                  <span className="flex-none w-5 h-5 rounded-full bg-evergreen-soft text-evergreen text-[11px] font-bold flex items-center justify-center mt-0.5">
                                    {i + 1}
                                  </span>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-ink leading-snug">{step.action}</p>
                        ...
                        {score === 5 && (
                          <p className="text-xs text-evergreen italic mt-2">
