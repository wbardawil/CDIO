...
**Decision (2026-05-13):** the Pre-Purchase Technology Audit is the recommended **first vendor-domain service to ship**, ahead of the full Lean Vendor Lifecycle. Sequence: ship the Audit Engine (~3-4 days) → run it on the founder's live ERP/CRM decisions as customer #0 → first hard `docs/OUTCOMES.md` entries → those entries sell both the Audit-as-a-service and the broader fractional engagement.

**Reprioritization (2026-05-13, later):** the Lean Vendor Lifecycle is **pulled forward from "deferred upsell" to a parallel priority track.** Trigger: customer #0 is actively advising **2 paying clients on live technology purchases now.** The Audit needs new prospects to monetize; the Purchase capability serves clients already paying — the faster path to a real Day-90 outcome, which is the metric this whole doc hangs on. This is the customer-#0 principle applied, not a scope violation. The scoring core (Selection Engine) already exists and is gated only by the v12 migration; the Lifecycle is the thin wrapper (Need Statement, RFP, references, memo, renewal) on top of it. Audit and Purchase are different engines, ONE app, ONE experience spine, ONE journey — built in parallel, both run on real clients. Execution plan: `docs/SESSION_HANDOFF.md` "Parallel track — Purchase".
